...
**Status: SOURCE REGISTER — KMA Weather and the nationwide Nongsaro occurrence bulletin are connected; crop-context Disease/Pest, Crop Information and Market remain candidates**
...
- 농사로 describes OpenAPI registration as phone identity verification, application approval, then issued key. The implemented `dbyhsCccrrncInfoList` endpoint returns title, author, registration date, view count and attachment metadata for nationwide occurrence bulletins. It does not establish a crop/cultivar/growth-stage match, so that mapping remains required for a later Disease/Pest or Crop Information adapter. [농사로 OpenAPI 안내](https://www.nongsaro.go.kr/portal/ps/psz/psza/contentMain.ps?menuId=PS00191)
...
|---|---|---|---|
| Account/key | KMA API Hub `authKey` | Occurrence bulletin key configured; crop-specific service still needs review | KAMIS key/requester ID or Public Data Portal key |
| Exact endpoint | Implemented current/short forecast; warning regional mapping pending | `dbyhsCccrrncInfoList` implemented for nationwide bulletin; crop-specific reference endpoint pending | Initial wholesale or retail endpoint and item/grade codes |
| Mapping | Farm area → KMA grid / forecast point | nationwide bulletin has no crop mapping; `crop_code`, cultivar, growth stage → official query remains pending | `crop_code` → KAMIS item/kind/market/grade code |
...
- Crop-context Disease/Pest is official information relevant to the selected crop, not a statement that a condition was found. The implemented nationwide occurrence bulletin is explicitly labelled as non-crop-specific reference material.
