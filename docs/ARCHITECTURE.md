...
현재 저장소에는 Core v0.1의 계획·Today·결과·문제·사진 첨부·후속·이력을 위한 Next.js 애플리케이션, Supabase migration, 워크스페이스 설정이 있습니다. 구현 범위는 `Email 로그인 또는 초대 링크 내 계정 설정 → Farm 생성·선택 → CropCycle 생성·선택·현재 생육 단계·종료 → TaskTemplate 적용 → FarmTask → 일정 → Today → ActionLog → IssueRecord → 선택적 사진 첨부 → Follow-up FarmTask → History`입니다. 선택 UI는 기존 RLS가 허용한 Farm과 CropCycle만 조회해, 기존 일정·Today·이력을 다시 불러오며 작업 계획을 자동으로 생성하지 않습니다. CropCycle 종료는 기존 `status`, `ended_at` 컬럼과 UPDATE RLS를 재사용하고, 작은 DB trigger가 종료 시각 기록·terminal 상태 보존을 강제합니다. 완료·취소 작기에는 작업 계획 생성 API와 UI를 모두 차단하지만 기존 데이터를 삭제하지 않습니다. 완료·문제 기록은 각각 PostgreSQL RPC로 ActionLog와 FarmTask 상태를 원자적으로 처리하며, 문제 RPC는 연결된 IssueRecord도 함께 생성합니다. 현재 생육 단계는 기존 `crop_cycles.growth_stage`를 권한 있는 사용자만 갱신하며 FarmTask를 자동 생성·재일정하지 않습니다. 사진은 결과와 분리된 Route Handler에서 비공개 Supabase Storage에 저장하고 Attachment 메타데이터를 기록합니다. Follow-up RPC는 원본 IssueRecord의 참조를 보존합니다. `src/proxy.ts`는 Supabase Auth 세션을 갱신해 Route Handler와 RLS가 동일한 로그인 사용자를 확인하도록 합니다. 코드 구조와 도구는 필요한 최소 단위로만 추가하며, 구조적 결정은 ADR에 기록합니다.

## Farm 협업 경계

Farm 협업은 기존 `farm_memberships`와 `owner`/`admin`/`farmer` 역할명을 재사용합니다. `farm_invitations`는 7일짜리 대기 초대와 해시된 토큰만 보관합니다. 초대·수락·역할 변경·제거는 브라우저의 직접 테이블 쓰기나 Service Role Key가 아니라 role-checked security-definer RPC로만 처리합니다. 다만 링크를 연 신규 사용자의 이메일·비밀번호 계정 설정은 브라우저의 Supabase Auth `signUp`으로 처리하며, 비밀번호는 Core API나 DB에 전달·저장하지 않습니다. owner는 admin·farmer를 초대하고 역할을 변경할 수 있으며, admin은 farmer만 초대·제거합니다. 이 최소 Slice는 기존 Farm·CropCycle·FarmTask·기록에 적용된 모든 구성원의 운영 RLS를 변경하지 않습니다.
