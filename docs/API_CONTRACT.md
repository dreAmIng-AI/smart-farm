...
GET /api/farms/{farmId}/weather-location returns the accessible Farm's saved label and KMA-grid context, or weatherLocation: null when configuration is not complete. Farm members may read this non-address forecast context; only owner/admin may save a label, gridX and gridY with the existing PATCH route. The normal browser flow selects a representative city/county/district entry from a bundled official KMA catalogue; no search input, street address or coordinate is sent to the API. If explicitly requested, the browser may use the device location internally to choose a KMA grid. The API never accepts or stores raw GPS coordinates or a street address.

`GET /api/farms/{farmId}/information/weather` returns a normalized KMA current-observation and short-forecast result for every Farm member. It uses the KMA API Hub’s Farm-grid endpoints, section 4.1 `getUltraSrtNcst` and section 4.3 `getVilageFcst`, through a server-only credential. Missing location context, provider access denial, other provider failures and stale-cache fallback return `200` with the user-safe `IntegrationResult` status; they never make Today task data fail.
...
`GET /api/farms/{farmId}/information/disease-pest/document?attachment={officialUrl}` lets a member read an official Nongsaro bulletin PDF inside the service. It first verifies Farm access, then accepts only an HTTPS Nongsaro attachment URL, follows only an official Nongsaro redirect, checks the PDF signature/type and streams at most 10 MB with `Content-Disposition: inline`. It is a reader proxy, not a PDF archive: it does not persist the file, expose arbitrary URLs or change the normalized bulletin result. The in-service reader adds `view=embed`; when an official PDF cannot be read, that mode returns only a Korean fallback page and the validated Nongsaro original link, never raw API JSON or a provider error.
