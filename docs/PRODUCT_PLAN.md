...
현재 Operations Core는 작업·기록 흐름, Today-first UX, FarmArea와 선택적으로 연결되는 CropCycle·FarmTask, 독립 Observation·수동 Measurement, Observation-origin IssueRecord, KMA 실제 Weather, 전국 단위 Nongsaro 병해충 발생정보와 Crop Pack으로 연결한 Nongsaro 작물별 참고자료까지 구현되어 있다. 품종·생육 단계까지 직접 맞춘 Disease/Pest, Market과 Pilot 데이터 검증은 별도 작은 PR로 추가한다. 따라서 v0.2 Pilot 준비 상태는 지금 `PARTIAL`이다.
...
7. KMA Weather → Nongsaro 전국 발생정보 → Crop Pack-mapped crop reference → cultivar/growth-stage context → Market의 실제 API 연결
