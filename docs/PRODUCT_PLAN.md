...
현재 Operations Core는 작업·기록 흐름, Today-first UX, FarmArea와 독립 Observation까지 구현되어 있다. Measurement, 실제 공공데이터와 Pilot 데이터 검증은 별도 작은 PR로 추가한다. 따라서 v0.2 Pilot 준비 상태는 지금 `PARTIAL`이다.
